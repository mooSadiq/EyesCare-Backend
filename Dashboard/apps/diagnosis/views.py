...
class DisagnosisListView(APIView):
    def get(self, request):
        ...
        diagnosis_serializer = DiagnosisSerializerDash(diagnosis, many=True,context={'request': request})
        return Response(
...
class ReportDiagnosisView(APIView):
    def get(self, request, pk):
        try:
            ...
            diagnosis_serializer = DiagnosisSerializerDash(report,context={'request': request})
            return JsonResponse(
                {"diagnosis": diagnosis_serializer.data}, status=status.HTTP_200_OK
            )
